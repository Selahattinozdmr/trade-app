...
export async function generateMetadata({
  ...
  return {
    title: `${userName} ile Mesajlaşma - Takas Go`,
    description: `${userName} ile mesajlaşın`,
  };
}
            ...
            <div className="animate-pulse">
              <div
                className={`max-w-xs h-12 rounded-lg ${
                  i % 2 === 0 ? "bg-orange-200" : "bg-gray-200"
                }`}
              ></div>
            </div>
          </div>
...
async function MessageThreadWrapper({
  currentUserId,
  otherUserId,
}: {
  ...
  const conversationResult = await getOrCreateConversation(otherUserId);

  if (!conversationResult.success || !conversationResult.data) {
    return (
      <div className="h-full flex items-center justify-center text-gray-500">
        <div className="text-center">
          <p className="text-sm">Konuşma yüklenirken hata oluştu</p>
  ...
  if (!messagesResult.success) {
    return (
      <div className="h-full flex items-center justify-center text-gray-500">
        <div className="text-center">
          <p className="text-sm">Mesajlar yüklenirken hata oluştu</p>
          <p className="text-xs text-gray-400 mt-1">{messagesResult.error}</p>
        </div>
      </div>
    );
  }
  ...
  if (!partnerResult.success || !partnerResult.data) {
    return (
      <div className="h-full flex items-center justify-center text-gray-500">
        <div className="text-center">
          <p className="text-sm">Kullanıcı bilgileri yüklenirken hata oluştu</p>
          <p className="text-xs text-gray-400 mt-1">{partnerResult.error}</p>
        </div>
      </div>
    );
  }
